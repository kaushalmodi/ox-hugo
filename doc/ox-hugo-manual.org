...
:END:
The "details" Org Special Blocks (~#+begin_details~ .. ~#+end_details~)
are used for generating the Details disclosure elements [[https://developer.mozilla.org/en-US/docs/Web/HTML/Element/details][~<details>~]]
and ~<summary>~.

/This is a *special* case of Org Special Blocks implemented only in
~ox-hugo~./

To separate the summary from the details:
- Use the summary-divider string ~---~ at the *beginning of a new
  line*.
...
- If the ~details~ Special Block does not contain the summary divider
  ~---~, it will be considered that the /summary/ part is not
  provided.
  #+begin_src org
  #+begin_details
  ...
  #+end_note
- If the last line in a ~details~ Special Block is *only the summary
  divider ~---~*, it will be considered that the /details/ part is not
  provided.
  #+begin_src org
  #+begin_details
  Some *Summary*
...
it can be made to show up opened by default by adding the ~open~
attribute like this:
#+begin_src org
#+attr_html: open
#+begin_details
