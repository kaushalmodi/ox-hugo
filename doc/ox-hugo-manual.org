...
3. Follow {{{pagelink(Installation,)}}} page to ensure that =ox-hugo=
...
5. Append these lines to the site config:
   ...
   [markup.goldmark.renderer]
     unsafe = true
   #+end_src
   See the page on {{{pagelink(Goldmark,)}}} for some more info on the
   =unsafe = true= setting.
...
2. Ensure that [[https://go.dev/doc/install][~go~]], and ~pandoc-citeproc~ (at least version
   {{{pandoc_version}}}) are installed (more info
   {{{pagelink(Requirements#code_contributor_reqs,here)}}}).
...
See the {{{pagelink(Usage#before-you-export,Usage → Before you
...
much Blackfriday-compatible now -- <2018-07-20 Fri>/
...
*** Meta
- Add documentation on how you can have {{{pagelink(Images in
...
** v0.5 <2017-11-06 Mon>
*** Features
- Export TOC as a Markdown ordered list. See {{{pagelink(Table of
  Contents,)}}} -- {{{issue(88)}}}.
...
See the new section added to documentation: {{{pagelink(Tags and
Categories,)}}}.
*** Features
...
- Source blocks can now be exported with line numbers and/or
  ...
  See {{{pagelink(Source blocks,)}}} for details.
